...
class Song {
    ...
    static async create(title, cover) {
        const result = await db.query(
            ...
            VALUES ($1, $2) RETURNING id `, [title, cover]);
    ...
    static async getByFilter(filters) {
        ...
        console.log(filters);
        const sqlQuery =
            `SELECT DISTINCT song.title FROM ${!joinStatements ? "song" : "setlist"}
            ${[...joinStatements].join(" ")}
           ${whereConditions}
    ...
    static async getCoversByFilter(filters) {
        let { joinStatements, whereConditions, limitCondition, offsetCondition, values } = sqlForSetlistFilters(filters);
        //Make sure to join on setlist if there are other joins
        if (joinStatements) {
            joinStatements.add(`INNER JOIN set ON setlist.id = set.setlist_id`);
            joinStatements.add(`INNER JOIN note ON set.id = note.set_id`);
        ...
        console.log(filters);
        const sqlQuery =
            `SELECT DISTINCT song.cover FROM ${!joinStatements ? "song" : "setlist"}
           ${[...joinStatements].join(" ")}
           ${whereConditions}
           ${!whereConditions ? 'WHERE' : 'AND'}
        ...
        console.log(sqlQuery);
        const coversRes = await db.query(
            sqlQuery, values);
