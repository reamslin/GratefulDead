...
class Venue {
    ...
    static async create(name, city, state, country) {
        const result = await db.query(
            ...
            VALUES ($1, $2, $3, $4) RETURNING id `, [name, city, state, country]);
    ...
    static async getByFilter(filters) {
        ...
        console.log(filters);
        const sqlQuery =
            `SELECT DISTINCT venue.name FROM ${!joinStatements ? "venue" : "setlist"}
            ${[...joinStatements].join(" ")}
           ${whereConditions}
    ...
    static async getStatesByFilter(filters) {
        ...
        console.log(filters);
        const sqlQuery =
            `SELECT DISTINCT venue.state FROM ${!joinStatements ? "venue" : "setlist"}
            ${[...joinStatements].join(" ")}
           ${whereConditions}
           ${!whereConditions ? 'WHERE' : 'AND'}
    ...
    static async getCitiesByFilter(filters) {
        ...
        console.log(filters);
        const sqlQuery =
            `SELECT DISTINCT venue.city FROM ${!joinStatements ? "venue" : "setlist"}
            ${[...joinStatements].join(" ")}
           ${whereConditions}
           ${!whereConditions ? 'WHERE' : 'AND'}
    ...
    static async getCountriesByFilter(filters) {
        ...
        console.log(filters);
        const sqlQuery =
            `SELECT DISTINCT venue.country FROM ${!joinStatements ? "venue" : "setlist"}
            ${[...joinStatements].join(" ")}
           ${whereConditions}
           ${!whereConditions ? 'WHERE' : 'AND'}
