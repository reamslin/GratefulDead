...
class Setlist {
  ...
  static async getByFilter(filters) {
    ...
    const setlistRes = await db.query(sqlQuery, values);
  ...
  static async getCount(filters) {
    ...
    const sqlQuery = `SELECT count(*) FROM setlist
            ${[...joinStatements].join(" ")}
            ${whereConditions}
            ${limitCondition} ${offsetCondition}`;
    const countRes = await db.query(sqlQuery, values);
  ...
  static async getYearsByFilter(filters) {
    ...
    const sqlQuery = `SELECT DISTINCT year FROM setlist
            ${[...joinStatements].join(" ")}
            ${whereConditions}
            ORDER BY year
            ${limitCondition} ${offsetCondition}`;
    const yearRes = await db.query(sqlQuery, values);
  ...
  static async getMonthsByFilter(filters) {
    ...
    const sqlQuery = `SELECT DISTINCT month FROM setlist
            ${[...joinStatements].join(" ")}
            ${whereConditions}
            ORDER BY month
            ${limitCondition} ${offsetCondition}`;
    const monthRes = await db.query(sqlQuery, values);
  ...
  static async getDaysByFilter(filters) {
    ...
    const sqlQuery = `SELECT DISTINCT day FROM setlist
            ${[...joinStatements].join(" ")}
            ${whereConditions}
            ORDER BY day
            ${limitCondition} ${offsetCondition}`;
    const dayRes = await db.query(sqlQuery, values);
