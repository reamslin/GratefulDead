...
const API_KEY = "6ZQ1mJ5Rr7iMWDUaTEBdVqS1lQMuY2l5-SE9";
...
async function getSetlistsPage(p) {
    try {
        const results = await axios.get(`${BASE_URL}/artist/${GD_MBID}/setlists?p=${p}`, config);
...
async function seedSetlists() {
    ...
    for (let setlist of setlists) {
        ...
        if (!venue_obj) {
            venue_obj = await Venue.create(setlist.venue.name, setlist.venue.city.name,
                setlist.venue.city.state, setlist.venue.city.country.name);
        }
        ...
        for (let set of setlist.sets.set) {
            ...
            for (let song of set.song) {
                ...
                let note_obj = await Note.create(set_obj.id, song_obj.id, song.info, with_artist)
            }
        }
    }
}
...
async function seed_recordings(setlist) {
    let q = `collection:${GD_Collection} AND title:'${setlist.year}-${setlist.month}-${setlist.day}'
     AND venue:${setlist.venue.name.split(" ").filter(w => w !== "The").join(" OR ")}`;
    let fields = 'taper, transferer, identifier, source, subject, date';

    const result = await axios.get(SCRAPE_BASE_URL, { params: { q, fields } });
    console.log(`Made scrape request for ${setlist.year}-${setlist.month}-${setlist.day}`);
    console.log(`Query: ${q}`);

    let items = result.data.items;

    for (let item of items) {
        if (item.date.slice(0, 10) === `${setlist.year}-${setlist.month}-${setlist.day}`) {
            await Recording.create(setlist.id, item.identifier, item.taper, item.transferer, item.source, item.subject);
