...
function SetlistList({ today }) {
    ...
    return (
        <div className={classes.root}>
            {!setlists ? <h1>Loading ...</h1>
                :
                <Paper className={classes.paper}>
                    <Typography variant='h3'>{!today ? Object.values(filters).join("-") : 'Today In Grateful Dead History'}</Typography>
                    <List className={classes.list}>{setlists.map(s => (<SetlistListItem setlist={s} />))}</List>
