        ...
        <div className={classes.root}>
            {!setlist ? <h1>Loading ... </h1>
                ...
                <Paper className={classes.paper}>
                    <Typography variant='h2'>{setlist.month}-{setlist.day}-{setlist.year}</Typography>
                    <Typography variant='h4'>{setlist.venue.name} </Typography>
                    <Typography variant='h6'> {setlist.venue.city}, {setlist.venue.state} </Typography>
                    <img className={classes.bears} src={stealie} width='50%' />
                    <div>
                        {setlist.sets.map(s => <Set set={s} key={s.id} />)}
                        <RecordingAccordion
